    ...
    switch (icode) {
      ...
      case I_RMMOVL: /* 4:0 regA:regB imm */
        if (NORM_REG(regA) && NORM_REG(regB)) {
            ...
            if (!set_long_val(sim->m,argB + imm,argA)) {
                err_print("PC = 0x%x, Invalid data address 0x%x", sim->pc,
                          argB + imm);
                return STAT_ADR;
            } 
            sim->pc = next_pc;
      ...
      case I_MRMOVL: /* 5:0 regB:regA imm */
        if (NORM_REG(regA) && NORM_REG(regB)) {
            long_t argB = get_reg_val(sim->r, regB);
            long_t valM = 0;
            if (!get_long_val(sim->m,argB + imm,&valM)) {
                err_print("PC = 0x%x, Invalid data address 0x%x", sim->pc,
                          argB + imm);
                return STAT_ADR;
            }
      ...
      case I_CALL: /* 8:x imm */
        {
          long_t val = get_reg_val(sim->r, REG_ESP);
      ...
      case I_RET: /* 9:0 */
        {
          long_t val = get_reg_val(sim->r, REG_ESP);
          long_t ret_adr = 0;
          if (!get_long_val(sim->m,val,&ret_adr)) {
      ...
      case I_PUSHL: /* A:0 regA:F */
        if (NORM_REG(regA) && NONE_REG(regB)) {
            ...
            if (!set_long_val(sim->m,val-4,argA)) {
                err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc,
                          val-4);
                return STAT_ADR;
            }
            sim->pc = next_pc;
